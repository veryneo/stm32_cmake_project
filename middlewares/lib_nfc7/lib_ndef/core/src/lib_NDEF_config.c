...
{
  if(bsp_nfc07a1_nfctag_data_read(pData, offset, length ) != D_BSP_RET_OK)
    return NDEF_ERROR;
  return NDEF_OK;
}
...
{
  if(bsp_nfc07a1_nfctag_data_write(pData, offset, length) != D_BSP_RET_OK)
    return NDEF_ERROR;
  return NDEF_OK;
}
...
{
  uint32_t size = 0;
  bsp_nfc07a1_nfctag_i2cUsrMemByteSize_get(&size);
  return size;
}
