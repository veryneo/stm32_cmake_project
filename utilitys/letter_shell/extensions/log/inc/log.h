...
//    #define LOG_TAG            __LINE__         /**< 自定添加的TAG */
#endif

#define BASIC_FILENAME 		   logGetFileBasicName(__FILE__)
...
#define     ERROR_TEXT          CSI(31) "E(%d) %s(%d):" CSI(39)     /**< 错误标签 */
#define     WARNING_TEXT        CSI(33) "W(%d) %s(%d):" CSI(39)     /**< 警告标签 */
#define     INFO_TEXT           CSI(32) "I(%d) %s(%d):" CSI(39)     /**< 信息标签 */
#define     DEBUG_TEXT          CSI(34) "D(%d) %s(%d):" CSI(39)     /**< 调试标签 */
#define     VERBOSE_TEXT        CSI(36) "V(%d) %s(%d):" CSI(39)     /**< 冗余信息标签 */
#else
...
const char* logGetFileBasicName(const char *fullpath);
...
#define logFormat(text, level, fmt, ...) \
        if (LOG_ENABLE) {\
            logWrite(LOG_ALL_OBJ, level, text " " fmt "" LOG_END, \
                LOG_TIME_STAMP, BASIC_FILENAME, __LINE__, ##__VA_ARGS__); }
